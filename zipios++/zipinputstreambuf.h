...
public:
  ...
  */
  explicit ZipInputStreambuf( std::streambuf *inbuf, int s_pos = -1, bool del_inbuf = false ) ;
...
private:
  ...
  ZipInputStreambuf( ZipInputStreambuf const& src ) ;

  /** Copy-assignment operator is private to prevent copying.  */
  ZipInputStreambuf const& operator = ( ZipInputStreambuf const& src ) ;
