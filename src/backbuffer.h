...
public:
  ...
  inline explicit BackBuffer( std::istream& is,
...
BackBuffer::BackBuffer( std::istream &is, VirtualSeeker vs, int chunk_size ) 
  : _vs        ( vs         )
  , _chunk_size( chunk_size )
  , _is        ( is         )
  , _file_pos  ( 0          )
{
  _vs.vseekg( is, 0, std::ios::end ) ;
...
int BackBuffer::readChunk( int& read_pointer )
{
  ...
  _chunk_size = std::min<int> ( static_cast< int >( _file_pos ), _chunk_size ) ;
  _file_pos -= _chunk_size ;
  _vs.vseekg( _is, _file_pos, std::ios::beg ) ;
