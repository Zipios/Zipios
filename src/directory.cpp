...
struct boost::filesystem::dir_it::representation
{
        ...
        //, m_stat() -- see below
    {
        ...
        else if(m_directory[m_directory.size() - 1] != '/')
        {
            m_directory += '/';
        }
    ...
    representation *release()
    {
        if(m_refcount <= 0)
        {
            throw std::runtime_error("boost::filesystem::dir_it::representation::release() called too many times");
        }
        return --m_refcount? 0: this;
    }
    ...
    {
        return (m_handle == 0) == (rhs.m_handle == 0);
    }
